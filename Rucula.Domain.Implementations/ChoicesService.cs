...
internal class ChoicesService : IChoicesService
{
    ...
    public async Task<ChoicesInfo> GetChoices(BondCommissions bondCommissions, WesternUnionParameters westernUnionParameters)
    {
        ...
        await Task.WhenAll(dolarBlueTask, dolarCryptoTask, dolarWesternUnionTask, dolarDiarcoTask).ConfigureAwait(false);

        var rankingTitulos = await _titulosService.GetNetCclRanking(dolarBlueTask.Result, bondCommissions).ConfigureAwait(false);

        return CreateWinningChoice(rankingTitulos, dolarBlueTask.Result, dolarCryptoTask.Result, dolarWesternUnionTask.Result, dolarDiarcoTask.Result);
    }
    ...
    public async Task<ChoicesInfo> RecalculateChoices(ChoicesInfo choices, BondCommissions bondCommissions, WesternUnionParameters westernUnionParameters)
    {
        ...
        return CreateWinningChoice(rankingTitulos, choices.Blue, choices.DolarCrypto, westernUnionDolar, choices.DolarDiarco);
    }

    private static ChoicesInfo CreateWinningChoice(IEnumerable<TituloIsin> rankingTitulos, Optional<Blue> dolarBlue, Optional<DolarCrypto> dolarCrypto, Optional<DolarWesternUnion> dolarWesternUnion, Optional<DolarDiarco> dolarDiarco)
    {
