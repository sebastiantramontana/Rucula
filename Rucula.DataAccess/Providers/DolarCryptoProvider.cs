...
internal class DolarCryptoProvider : IDolarCryptoProvider
{
    ...
    public async Task<Optional<DolarCrypto>> GetCurrentDolarCrypto()
    {
    ...
    private Optional<DolarCryptoDto> ConvertContentToCrypto(string content)
        => _dolarCryptoJsonDeserializer
            .Deserialize(JsonNode.Parse(content)!);

    private Optional<DolarCrypto> MapToCrypto(Optional<DolarCryptoDto> dto)
